...
const doctorData = [
  { id: "1", name: "Dr. Raj Malhotra", specialization: "Cardiologist", status: "Active" },
  { id: "2", name: "Dr. Ananya Sen", specialization: "Neurologist", status: "Active" },
  { id: "3", name: "Dr. Arjun Patel", specialization: "Orthopedic", status: "Blocked" },
  { id: "4", name: "Dr. Meera Iyer", specialization: "Dermatologist", status: "Active" },
]
      ...
      <div className="ml-64 px-8 py-6">
        ...
        <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4 mb-8">
          ...
          <ul className="text-sm text-yellow-700 space-y-1">
            ...
            <li>• {blockedPatients} patient account(s) are currently blocked.</li>
        ...
        <div className="bg-white border rounded-lg p-4 mb-10">
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            <Link href="/admin/doctorList" className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-700">
              Manage Doctors
            </Link>

            <Link href="/admin/patientList" className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-700">
              Manage Patients
            </Link>

            <Link href="/admin/reports" className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-700">
              View Reports
            </Link>
          </div>
        </div>
              ...
              <tbody>
                {doctorData.map((doc) => (
                  <tr key={doc.id}>
                    ...
                    <td className={`p-2 border font-medium ${doc.status === "Active" ? "text-green-600" : "text-red-600"}`}>
                      {doc.status}
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {patientData.map((pat) => (
                  <tr key={pat.id}>
                    ...
                    <td className={`p-2 border font-medium ${pat.status === "Active" ? "text-green-600" : "text-red-600"}`}>
                      {pat.status}
                    </td>
                  </tr>
                ))}
